...
class TwitterAPIClient: BDBOAuth1SessionManager {
    ...
    func handleOpenUrl(url: URL, success:@escaping (Bool) -> ()){
...
//                self.loginSuccess?()
    ...
    func mentions(success: @escaping (_ tweets:[Tweet]) -> (), failure: @escaping (_ error:Error) -> ()) {
    ...
    func userTimeline(id: UInt64, success: @escaping (_ tweets:[Tweet]) -> (), failure: @escaping (_ error:Error) -> ()) {
        get("1.1/statuses/user_timeline.json?user_id=\(id)", parameters: nil, success: { (task: URLSessionDataTask, response: Any?) in
            print("\(response)")
            let responseDictionary = response as? [Dictionary<String, Any>]
            let tweets = Tweet.getTweetArray(tweetDictionaries: responseDictionary!)
            success(tweets)
